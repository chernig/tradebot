...
Linear Regression Channel strategy works based on identifying potential price channels and trading the channel. 
...
class strategy_linear_regression_channel:
    def __init__(self, symbol, timeframe, lrc_std_1, lrc_period_1, lrc_std_2, lrc_period_2, lrc_std_3, lrc_period_3, lrc_std_exit, lrc_period_exit, cmf_period, cmf_ma_period, r_percent_period):
        self.symbol=symbol
    ...
    def check_exit(self, current_position):
        try:
            self.get_data()
            linear_regression_exit, upper_line_exit, lower_line_exit, slope_exit, channel_width_exit=linear_regression_channel(self.data, self.lrc_period_exit, self.lrc_std_exit)
    ...
    def check_entry(self):
        try:
            self.get_data()
            linear_regression_1, upper_line_1, lower_line_1, slope_1, channel_width_1=linear_regression_channel(self.data, self.lrc_period_1, self.lrc_std_1)
            linear_regression_2, upper_line_2, lower_line_2, slope_2, channel_width_2=linear_regression_channel(self.data, self.lrc_period_2, self.lrc_std_2)
            linear_regression_3, upper_line_3, lower_line_3, slope_3, channel_width_3=linear_regression_channel(self.data, self.lrc_period_3, self.lrc_std_3)
            self.data['cmf']=cmf(list(self.data.bidclose), list(self.data.bidhigh), list(self.data.bidlow), list(self.data.tickqty), self.cmf_period)
            self.data['cmf_ma']=ema(list(self.data.cmf), self.cmf_ma_period)
            self.data['r_percent']=r_percent(self.data, self.r_percent_period)
    ...
    def backtest_exit(self, current_position, data):
        try:
            ...
            if len(data.date)>exit*2:
                ...
                linear_regression_exit, upper_line_exit, lower_line_exit, slope_exit, channel_width_exit=linear_regression_channel(data, self.lrc_period_exit, self.lrc_std_exit)
    ...
    def backtest_entry(self, data):
        try:
            ...
            if len(data.date)>max(self.lrc_period_1, self.lrc_period_2, self.lrc_period_3):
                data=data.iloc[(len(data.date)-(2*max(self.lrc_period_1, self.lrc_period_2, self.lrc_period_3))):]
                linear_regression_1, upper_line_1, lower_line_1, slope_1, channel_width_1=linear_regression_channel(data, self.lrc_period_1, self.lrc_std_1)
                linear_regression_2, upper_line_2, lower_line_2, slope_2, channel_width_2=linear_regression_channel(data, self.lrc_period_2, self.lrc_std_2)
                linear_regression_3, upper_line_3, lower_line_3, slope_3, channel_width_3=linear_regression_channel(data, self.lrc_period_3, self.lrc_std_3)
                data['cmf']=cmf(list(data.bidclose), list(data.bidhigh), list(data.bidlow), list(data.tickqty), self.cmf_period)
                data['cmf_ma']=ema(list(data.cmf), self.cmf_ma_period)
                data['r_percent']=r_percent(data, self.r_percent_period)
