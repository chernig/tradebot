...
class Db_Controller():
    ...
    def query_open_positon_dashboard(self):
        ...
        try:
            ...
            query = "Select positionMaker, currency, avg(grossPl), avg(visiblePL), max(amountK), avg(amountK), min(amountK), count(tradeId), count_successful_trades(visiblePL), count_unsuccessful_trades(visiblePL)   from OpenPosition group by positionMaker, currency order by visiblePL, grossPL"
    ...
    def query_closed_positon_dashboard(self):
        ...
        try:
            ...
            query = "Select positionMaker, currency, avg(grossPl), avg(visiblePL), max(amountK), avg(amountK), min(amountK), count(tradeId), count_successful_trades(visiblePL), count_unsuccessful_trades(visiblePL)   from ClosedPosition group by positionMaker, currency order by visiblePL, grossPL"
            cursor.execute(query)
            result=cursor.fetchall()
            self.db_close_connection(connection)
            return result
